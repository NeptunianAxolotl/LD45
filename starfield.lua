...
local lprime = 1031
local fields = {{id = 1, bsize = 128, minf = 10, maxf = 25, probs = {0.9,0.6,0.3,0.1}},{id = 2, bsize = 128, minf = 25, maxf = 100, probs = {0.5,0.2,0.1}},{id = 3, bsize = 128, minf = 100, maxf = 1000, probs = {0.25,0.05}}} -- determines z-distance and frequency of stars
...
local function starsInBox(bx,by,field)
  ...
  if not starcache[field.id] then
	starcache[field.id] = {}
  end
  
  if not starcache[field.id][bx] then
	starcache[field.id][bx] = {}
  end
  
  if not starcache[field.id][bx][by] then
	...
	starcache[field.id][bx][by] = stars
  end
  
  return starcache[field.id][bx][by]
end
...
local function locations(px, py, cameraScale)
  ...
  for f = 1, #fields do
	...
	-- stop storing any stars not currently close to us
	for k2,v2 in pairs(starcache[fields[f].id]) do
      for k3,v3 in pairs(v2) do
	    if (k2 < x1 - 2) or (k2 > x2 + 2) or (k3 < y1 - 2) or (k3 > y2 + 2) then
		  v2[k3] = nil
		end
	  end
	  if next(v2) == nil then
		starcache[fields[f].id][k2] = nil
      end
	end
  end
