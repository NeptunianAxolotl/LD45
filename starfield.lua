...
local lprime = 1031
local fields = {{bsize = 128, minf = 10, maxf = 25, probs = {0.9,0.6,0.3,0.1}},{bsize = 128, minf = 25, maxf = 100, probs = {0.5,0.2,0.1}},{bsize = 128, minf = 100, maxf = 1000, probs = {0.25,0.05}}}
local winkProb = 0.25
...
local function starsInBox(bx,by,field)
  ...
  for i = 1, nstars do
	...
	stars[#stars + 1] = {x=sx,y=sy,z=sz,mag=sm}
  end
...
local function locations(px, py, cameraScale)
  ...
  local cutoff = (1 - 1 / (cameraScale * cameraScale)) * 0.99
		...
		for j = y1, y2 do
			...
			for s = 1, #stars do
				...
				if bright > 0.05 then 
					...
					onscreenStars[#onscreenStars+1] = {prjx, prjy, light, light, light, 1}
				end
			end
		end
	  end
